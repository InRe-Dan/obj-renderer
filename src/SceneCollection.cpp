...
Scene* SceneCollection::makeCornellScene()
{
	...
	Light* light2 = new Light(
		...
		true,
		false);
	scene->addAnimation(new Translation(
		camera,
		[](vec3 origin, int tick) {
			return vec3(
				2 * glm::cos(tick / 24.0f),
				0,
				2 * glm::sin(tick / 24.0f));
		}));
...
Scene* SceneCollection::makeTexturedCornellScene()
{
	...
	scene->addLight(
		new Light("Red", vec3(-2.2, 2.2, 3), 5, Colour(255, 0, 0), true));
	scene->addLight(
		new Light("Green", vec3(2.2, -2.2, 3), 5, Colour(0, 255, 0), true));
	scene->addLight(
		new Light("Blue", vec3(-2.2, -2.2, 3), 5, Colour(0, 0, 255), true));
...
Scene* SceneCollection::makeLogoScene()
{
	...
	{
		// For the first 5 seconds, pan camera backwards from scene as two
		// lights race by
		if (0 <= tick && tick < 24 * 5)
		{
	...
	std::function<vec3(vec3, int)> cameraPositionFunction =
		[](vec3 origin, int tick)
	{
	...
	std::function<vec3(vec3, int)> whiteLightFunction =
		[](vec3 origin, int tick)
	{
		...
		else if (10 * 24 <= tick && tick < 24 * 15)
		{
			...
			return vec3(2.5, 2.5, 3);
	};
	std::function<vec3(vec3, int)> redLightFunction =
		[](vec3 origin, int tick)
	{
	...
	std::function<vec3(vec3, int)> blueLightFunction =
		[](vec3 origin, int tick)
	{
	...
	Light* whiteLight = new Light(
		...
		0.1);
	Light* red =
		new Light("Red", vec3(1, 0, 2), 2, Colour(255, 0, 0), true, false);
	...
	scene->addAnimation(
		new AdjustableRotation(camera, cameraRotationFunction));
