...
MaterialLib::MaterialLib(const std::filesystem::path& filename)
{
	...
	while (std::getline(buffer, line))
	{
		...
		{
			materials[materialName]->setDiffuseColour(glm::vec4(parseTriplet(line), 1.0f));
		}
		else if (code.compare("Ka") == 0)
		{
			materials[materialName]->setAmbientColour(glm::vec4(parseTriplet(line), 1.0f));
		}
		else if (code.compare("Ks") == 0)
		{
			materials[materialName]->setSpecularColour(glm::vec4(parseTriplet(line), 1.0f));
		}
		else if (code.compare("Ns") == 0)
		{
			std::vector<std::string_view> lineSplit = split(line, ' ');
			float exponent(std::stof(std::string(lineSplit.at(1)).data()));
