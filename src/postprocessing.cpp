...
Kernel::Kernel(vector<vector<float>> kernelTemplate)
	: vect(kernelTemplate)
	, size(kernelTemplate.size())
	, nFactor(
		  [](vector<vector<float>> kernelTemplate)
		  {
			  float sum = 0.0f;
...
applyKernel(vector<vector<uint32_t>>& target, const Kernel& k)
{
...
void blackAndWhite(vector<vector<uint32_t>>& target)
{
	...
	for (int i = 0; i < targetH; i++)
	{
		for (int j = 0; j < targetW; j++)
		{
			...
			// percieves different channels. Source: e2eml.school - Course 137
			uint32_t sum = ((col >> 16) & 0x000000FF) * 0.299 +
						   ((col >> 8) & 0x000000FF) * 0.587 +
						   (col & 0x000000FF) * 0.114;
			sum = sum > 255 ? 255 : sum;
...
void hypot(
	...
	vector<vector<uint32_t>>& one,
	vector<vector<uint32_t>>& two)
{
	int targetW = target.at(0).size();
	int targetH = target.size();
	for (int i = 0; i < targetH; i++)
	{
		for (int j = 0; j < targetW; j++)
		{
			...
			uint8_t r = glm::min(
				255,
				roundI(255.0f * glm::length(vec2(r1, r2)) / maxLength));
	...
	vector<vector<uint32_t>>& target)
{
	...
	for (int i = 0; i < target.size(); i++)
	{
		for (int j = 0; j < target.at(0).size(); j++)
		{
			...
			if (nearestx > sWidth - 1)
				nearestx = sWidth - 1;
			if (nearestx < 0)
				nearestx = 0;
			if (nearesty > sHeight - 1)
				nearesty = sHeight - 1;
			if (nearesty < 0)
				nearesty = 0;
	...
	vector<vector<uint32_t>>& target)
{
	...
	for (int i = 0; i < target.size(); i++)
	{
		for (int j = 0; j < target.at(0).size(); j++)
		{
			...
			if (higherx > sWidth - 1)
				higherx = sWidth - 1;
			if (highery > sHeight - 1)
				highery = sHeight - 1;
			uint8_t* argb1 = (uint8_t*) &(source[lowery][lowerx]);
			uint8_t* argb2 = (uint8_t*) &(source[highery][higherx]);
			for (int channel = 1; channel < 4; channel++)
			{
				// Values here should be weighted and both dimensions (not just
				// diagonal) should be used
